...
import { 
  Calendar as CalendarIcon, Search, MapPin, User, Users, 
  ChevronLeft, ChevronRight 
} from "lucide-react";
import { 
  format, startOfWeek, addDays, isSameDay, parseISO, getHours, 
  startOfMonth, endOfMonth, eachDayOfInterval, isSameMonth,
  addMonths, subMonths, addWeeks, subWeeks, subDays
} from "date-fns";
                ...
                <div className="w-full">
                    ...
                    >
                        <div className="flex flex-col items-center gap-1 mb-3">
                            <h3 className="font-bold text-xl text-gray-800">{currentTrip.name}</h3>
                            <p className="text-sm text-gray-500 font-medium">{currentTrip.destination}</p>
                        </div>
                        
                        <div className="pt-3 border-t border-[#94C3D2]/20 w-full">
                            <div className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-3 text-center">Shared With</div>
                            <div className="flex flex-wrap justify-center gap-2">
                                {currentTrip.contributors.map((user, i) => (
                                    <div key={i} className="flex items-center gap-1.5 bg-white px-2 py-1 rounded-full border border-[#94C3D2]/30 shadow-sm" title={user.userEmail}>
                                        ...
                                        <span className="text-xs text-gray-600 font-medium max-w-[80px] truncate">{user.userName}</span>
                                    </div>
                                ))}
        ...
        <main className="flex-1 w-full bg-white rounded-[2.5rem] shadow-xl border border-gray-100 overflow-hidden flex flex-col">
                ...
                <div className="flex flex-col md:flex-row md:items-center gap-4">
                    ...
                    <div className="flex items-baseline gap-2">
                        <h1 className="text-2xl font-bold text-gray-800">
                            {view === 'day' ? format(currentDate, "MMM d") : format(currentDate, "MMMM")}
                        </h1>
                    ...
                    <div className="flex bg-gray-100 rounded-lg p-1 w-fit">
                        <button onClick={handlePrev} className="p-1.5 hover:bg-white rounded-md transition-colors text-gray-600">
                            <ChevronLeft size={18} />
                        </button>
                        <div className="w-px bg-gray-200 my-1 mx-1"></div>
                        <button onClick={handleNext} className="p-1.5 hover:bg-white rounded-md transition-colors text-gray-600">
            ...
            <div className="flex items-center gap-4">
                <div className="relative hidden xl:block">
                    ...
                    <input type="text" placeholder="Search events..." className="pl-9 pr-4 py-2 bg-gray-50 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#94C3D2]"/>
                </div>
            </div>
            </div>
            ...
            {view === 'month' ? (
                ...
                {calendarDays.map((day, i) => {
                    ...
                    return (
                    <div key={i} className={`min-h-[120px] bg-white p-2 flex flex-col gap-1 hover:bg-gray-50 transition-colors ${!isSameMonth(day, currentDate) ? 'bg-gray-50/50' : ''}`}>
                        <div className={`text-right mb-1 ${isSameDay(day, new Date()) ? 'text-blue-600 font-bold bg-blue-50 w-7 h-7 rounded-full flex items-center justify-center ml-auto' : 'text-gray-600 p-1'}`}>
                        {format(day, 'd')}
                        </div>
                        {dayEvents.map(ev => (
                        <div key={ev.id} className="text-[10px] bg-blue-100 text-blue-700 px-2 py-1 rounded-md truncate font-medium border-l-2 border-blue-500 hover:shadow-sm cursor-pointer transition-shadow">
                            {ev.title}
                        </div>
                        ))}
                    </div>
                ...
                <div className={`flex-1 grid ${view === 'day' ? 'grid-cols-1' : 'grid-cols-7'} relative`}>
                    ...
                    <div key={i} className="text-center pb-4 border-b border-gray-100">
                        ...
                        isSameDay(day, new Date()) ? "bg-black text-white" : "text-gray-800"
                        }`}>
                        {format(day, "d")}
                        </div>
                    </div>
                    ...
                    <div key={dayIndex} className="relative border-r border-gray-50 last:border-r-0 pt-2 bg-white">
                        {hours.map((h, i) => (
                        ...
                        .map((event, eventIdx) => {
                            ...
                            const colors = ["bg-emerald-100 border-emerald-200 text-emerald-800", "bg-indigo-100 border-indigo-200 text-indigo-800", "bg-amber-100 border-amber-200 text-amber-800"];
                            ...
                            >
                                ...
                                <Users size={10} />
                                <span className="truncate text-[10px]">Group Event</span>
                                </div>
                            </div>
                            );
                        })}
