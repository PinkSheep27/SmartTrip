...
export default function ManageTripPage() {
    const { activeCartId, setActiveCartId } = useTrip(); // Added setActiveCartId in case we delete the active trip
    ...
    const handleUpdateTrip = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await fetchTripData(); // Refresh the data to show changes
            setIsEditing(false);
        } catch (error) {
    ...
    const handleDeleteTrip = async () => {
        ...
        try {
            ...
            alert("Trip deleted.");
            setActiveCartId(null); // Clear the active cart
    ...
    const dynamicCollaborators = tripDetails?.participants?.length > 0
        ...
        : [{ id: 'me', name: 'Me', initials: 'ME', color: 'bg-blue-600' }];
    ...
    const perPerson = total / (dynamicCollaborators.length || 1);
    ...
    return (
        <div className="min-h-screen bg-gray-50 p-4 md:p-8 pt-28 md:pt-32 font-sans text-gray-900">
            <div className="max-w-6xl mx-auto">
                ...
                {/* --- MODIFIED: Page Header with Edit Form --- */}
                        ...
                        {isEditing ? (
                            <form onSubmit={handleUpdateTrip} className="w-full">
                                ...
                                <div className="flex flex-col-reverse sm:flex-row justify-between items-center gap-4">
                                    {/* Delete Trip is now hidden inside the Edit Menu */}
                            ...
                            <div className="w-full">
                                <div className="flex items-center gap-3 mb-2">
                                    ...
                                    {/* Subtle Pencil Icon next to the title */}
                                    <button
                    ...
                    <div className="flex items-center gap-2 mt-6 pt-4 border-t border-gray-100">
                        ...
                        <div className="flex -space-x-2">
                            {dynamicCollaborators.map((user: any) => (
                                <div key={user.id} className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold border-2 border-white shadow-sm ${user.color}`} title={user.name}>
                                    {user.initials}
                                </div>
                            ))}
                        </div>
                        <button className="w-8 h-8 rounded-full border-2 border-dashed border-gray-300 text-gray-400 hover:text-blue-600 hover:border-blue-300 flex items-center justify-center transition-colors ml-1 bg-gray-50">
                            +
                        </button>
                    </div>
                </div>
                        ...
                        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-6 sticky top-32">
                            ...
                            <div className="border-t border-gray-100 pt-4 mb-6">
                                ...
                                <div className="flex justify-between text-sm text-blue-600 bg-blue-50 p-2 rounded-lg mt-3">
                                    <span className="font-medium">Split between {dynamicCollaborators.length}</span>
